    ...
    $(function() {
      if (result.status == 1 && result.data.logged_in ===
        ...
        if (result.data.eligibility ==
          "eligible") {
          $('#textEligibile').text(
            'Eligible for prizes').addClass(
            "success-text");
        } else {
          $('#textEligibile').text(
            'Not eligible for prizes').addClass(
            "failure-text");
        }
      }
    });
...
$(function() {
  ...
  $('#headerProfileMobile').addClass("active");
  $('.navbar').addClass('profile');
  $('#inputCountdown').click(function() {
    localStorage.setItem("nocountdown", $('#inputCountdown')
      .is(':checked'));
